...
exports.mentorReviewsGet = async (id) => {
    ...
    while (1) {
        ...
        if (index === -1) {
            ...
            evaluation_key.push(index);
            if (evaluation_key.length === 3) break;
        }
        index++;
    }
    ...
    // 품목 정렬 (높은 평가 항목 순대로)
    let evaluation = [];
    let evaluation_info = await Mentorevaluation.findAll({ where: { id: { [Op.or]: evaluation_key } } });
    evaluation_key.forEach((key) => {
        ...
        evaluations: [
            {
                ...
                percentage: (count[evaluation_key[0]] / review_count).toFixed(2) * 100,
                count: count[evaluation_key[0]],
            },
            {
                evaluation: evaluation[1],
                percentage: (count[evaluation_key[1]] / review_count).toFixed(2) * 100,
                count: count[evaluation_key[1]],
            },
            {
                evaluation: evaluation[2],
                percentage: (count[evaluation_key[2]] / review_count).toFixed(2) * 100,
                count: count[evaluation_key[2]],
            },
        ],
    });
